describe('Salad Bar', () => {
  ...
  describe('Personalise salad', () => {
    beforeEach(() => {
      cy.intercept('GET', '**/toppings').as('getToppings')
    ...
    it('Verify /toppings API', () => {
      ...
      cy.wait('@getToppings').should(({ request, response }) => {
        ...
        if (response?.statusCode === 200) {
          expect(response?.body).to.have.property('length').and.equal(12)
        }
      })
    })
    ...
    it('select first topping', () => {
      ...
      cy.get('ul[data-cy="topping-list"] li').first().find('button').click()
    })
  })
})
